  ...
  class ShippingRate < ActiveRecord::Base
    ...
    def price_criteria_check
      if price_from.present? && price_to.present? && criteria == "weight" && !weight_from.present? && !weight_to.present?
        errors.add(:criteria, I18n.t('activerecord.models.shipping_rate.not_correct'))
      else
        return
      end
    end
    ...
    def price_comparison_check
      if price_to.present? && price_from.present? && price_to < price_from
        errors.add(:price_to, I18n.t('activerecord.models.shipping_rate.price_to_must_be_greater'))
      else
        return
      end
    end
    ...
    def weight_comparison_check
      if weight_from.present? && weight_to.present? && weight_to < weight_from
        errors.add(:weight_to, I18n.t('activerecord.models.shipping_rate.weight_to_must_be_greater'))
      else
        return
      end
    end
