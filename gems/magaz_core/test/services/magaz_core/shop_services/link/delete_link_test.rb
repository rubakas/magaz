...
class MagazCore::ShopServices::Link::DeleteLinkTest < ActiveSupport::TestCase
  ...
  test 'should delete link with valid id' do
    service = MagazCore::ShopServices::Link::DeleteLink.run(id: @link.id)
    assert service.valid?
    assert_equal 0, MagazCore::Link.count
  end

  test 'should not delete link with blank id' do
    service = MagazCore::ShopServices::Link::DeleteLink.run(id: '')
