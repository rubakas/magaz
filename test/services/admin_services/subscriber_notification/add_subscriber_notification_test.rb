...
class AdminServices::SubscriberNotification::AddSubscriberNotificationTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @phone_params = {shop_id: @shop.id, subscriber_notification_params: {
                     notification_method: "phone",
                     subscription_address: "9379992"}}
    @email_params = {shop_id: @shop.id, subscriber_notification_params: {
                     notification_method: "email",
                     subscription_address: "valid@email.com"}}
    @blank_params = {shop_id: @shop.id, subscriber_notification_params: {
                     notification_method: "",
  ...
  test "should add subscriber notification with valid phone params" do
    ...
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@phone_params).run
    assert service.success?
    assert SubscriberNotification.find(service.subscriber_notification.id)
    assert_equal 2, SubscriberNotification.count
    assert_equal @phone_params[:subscriber_notification_params][:subscription_address],
  ...
  test "should add subscriber notification with valid email params" do
    assert_equal 1, SubscriberNotification.count
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@email_params).run
    assert service.success?
    assert SubscriberNotification.find(service.subscriber_notification.id)
    assert_equal 2, SubscriberNotification.count
    assert_equal @email_params[:subscriber_notification_params][:subscription_address],
      SubscriberNotification.find(service.subscriber_notification.id).subscription_address
  end
  ...
  test "shoul not add subscriber notification with invalid phone" do
    @phone_params[:subscriber_notification_params][:subscription_address] = "not a number"
    assert_equal 1, SubscriberNotification.count
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@phone_params).run
    refute service.success?
    assert_equal 1, service.subscriber_notification.errors.full_messages.count
  ...
  test "shoul not add subscriber notification with invalid email" do
    @email_params[:subscriber_notification_params][:subscription_address] = "wrong email"
    assert_equal 1, SubscriberNotification.count
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@email_params).run
    refute service.success?
    assert_equal 1, service.subscriber_notification.errors.full_messages.count
  ...
  test "shoul not add subscriber notification with too long email" do
    @email_params[:subscriber_notification_params][:subscription_address] = "loooooooooooooooooong@email.com"
    assert_equal 1, SubscriberNotification.count
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@email_params).run
    refute service.success?
    assert_equal 1, service.subscriber_notification.errors.full_messages.count
  ...
  test "shoul not add subscriber notification with existing email" do
    @email_params[:subscriber_notification_params][:subscription_address] = @subscriber_notification.subscription_address
    assert_equal 1, SubscriberNotification.count
    service = AdminServices::SubscriberNotification::AddSubscriberNotification.new(@email_params).run
    refute service.success?
    assert_equal 1, service.subscriber_notification.errors.full_messages.count
